...
public class SpatialHashGrid {
	...
	@SuppressWarnings("unchecked")
	public SpatialHashGrid(float worldWidth,float worldHeight,float cellSize,int capacity){
		this.cellSize = cellSize;
		this.cellsPerRow = (int)Math.ceil(worldWidth/cellSize);
		this.cellsPerCol = (int)Math.ceil(worldHeight/cellSize);
		int numCells = cellsPerRow * cellsPerCol;
		...
		staticCells = new List[numCells];
		for(int i=0; i<numCells; i++){
			dynamicCells[i] = new ArrayList<GameObject>(capacity);
			staticCells[i] = new ArrayList<GameObject>(capacity);
		}
		foundObjects = new ArrayList<GameObject>(capacity);
	}
	...
	public void clearDynamicCells(Object obj){
